...
rule all:
    input:
        # expand('data/demux/demultiplex.{barcodes}.bam',barcodes=barcodes)       # expand("data/aligned/{sample}.sorted.bam",sample=samps),
        expand('data/aligned/{sample}.bam',sample=samps)       # expand("data/aligned/{sample}.sorted.bam",sample=samps),
        # "qc/multiqc.html",
        # 'qc/all_samples.csv'
...
module ref:
    snakefile: "../../reference_format/Snakefile.smk"
    config: config
