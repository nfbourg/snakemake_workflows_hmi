...
def match_sample_sheets(input_data_dirs,sample_sheets,bad_run=False):
    samp_sheet_dict = {}
    samp_dict = {}
    samps = []
    for indir, samp_sheet in zip(input_data_dirs,sample_sheets):
        samp_sheet_dict[indir] = samp_sheet
        if not bad_run:
            sub_samps =  read_sample_sheet(samp_sheet)['Sample_Name'].tolist()
        else:
